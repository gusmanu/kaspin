    ...
    <form method="POST" action="{{route('edit', $product->id)}}">
        ...
        <div class="form-row">
            <div class="form-group col-md-6 no-margin-bottom">
                ...
                <input type="text" name="nama" id="nama" class="form-control" value="@if(old('nama')){{old('nama')}}@else{{$product->nama}}@endif" placeholder="Nama Produk">
                @error('nama')
        ...
        <div class="form-row">
            ...
            <div class="form-group col-md-6 no-margin-bottom">
                ...
                <select class="form-control" name="status" id="status" required="">
                    <option value="aktif" @if(old('status') and old('status') == 'aktif') selected @elseif ($product->status == 'aktif') selected @endif>Aktif</option>
                    <option value="nonaktif" @if(old('status') and old('status') == 'nonaktif') selected @elseif ($product->status == 'nonaktif') selected @endif>Nonaktif</option>
                </select>
                @error('status')
                <div class="invalid-feedback" style="display: block">
                    {{$message}}
